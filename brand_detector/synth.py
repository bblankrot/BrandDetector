...
def generate_synthetic_df(df_train, n_synth):
    n_synth = min(n_synth, df_train.shape[0])
    synth_list = generate_synthetic_names(n_synth)
    df_synth = df_train[["brand", "transcription"]].sample(n_synth).copy()
    i = 0  # cannot iterate over row index as it might be a string
